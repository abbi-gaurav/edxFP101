...
euclid :: (Int,  Int) -> Int
euclid (x, y) = if (x == y) then
                    x
                else
                    if(x < y) then euclid(x, y-x) else euclid(x-y, y)
...
funkyMap f g xs = funkyMapL f g 0 xs

funkyMapL :: (a->b) -> (a -> b) -> Int -> [a] -> [b]
funkyMapL f g n [] = []
funkyMapL f g n (x:xs) =  mv : funkyMapL f g (n+1) xs
                          where mv = if (even n ) then f x else g x
