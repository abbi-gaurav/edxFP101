...
toDigits n
         | d > 0 = toDigits d ++ [m]
         | d == 0 = [m]
         | otherwise = error "number less than 0"
                       where d = n `div` 10
                             m = n `mod` 10
...
toDigitsRev n
            | d > 0 = m : toDigitsRev (d)
            | d == 0 = [m]
            | otherwise = error "n is less than 0"
                          where d = n `div` 10
                                m = n `mod` 10
-- ===================================
-- Ex. 2
...
doubleSecond xs = [doubleOrNot x n | (x,n) <- zip xs [1..]]
                  where doubleOrNot x n = if (even n) then 2 * x else x
